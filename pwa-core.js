...
document.addEventListener('DOMContentLoaded', function onDOMReady() {
  ...
  if (!isIOS && !isStandalone) {
    ...
    let shown = false;
    let installPromptFired = false; // Chrome có fire beforeinstallprompt không?
    ...
    window.addEventListener('beforeinstallprompt', (e) => {
      // Chrome chỉ fire event này khi app CHƯA được cài.
      // Nếu đã cài rồi, event này sẽ không bao giờ fire.
      e.preventDefault();
      installPromptFired = true;
      deferredPrompt = e;

      // Đã cài (flag localStorage) thì không hiện overlay
    ...
    // Fallback: nếu sau 3s vẫn không có beforeinstallprompt
    // → khả năng (A) đã cài rồi, hoặc (B) chưa đủ điều kiện PWA
    // Chỉ hiện overlay nếu chưa có flag "đã cài"
    setTimeout(() => {
      if (installPromptFired) return; // event đã xử lý rồi, bỏ qua
      if (hasInstalled()) return;     // đã cài → không hiện
      showAndroid();                  // chưa cài, không có prompt → hiện hướng dẫn thủ công
    }, 3000);
