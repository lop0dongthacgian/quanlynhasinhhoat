...
document.addEventListener('DOMContentLoaded', function onDOMReady() {
  ...
  if (isIOS && !isStandalone) {
    const installed = (() => { try { return localStorage.getItem('pwa-installed')==='1'; } catch(_){return false;} })();
    if (!installed) setTimeout(() => showOverlay('ios'), cfg.IOS_DELAY);
  }
  ...
  if (!isIOS && !isStandalone) {
    ...
    if (!installed) {
      ...
      // Fallback: nếu không có beforeinstallprompt sau 3s → vẫn hiện overlay hướng dẫn
