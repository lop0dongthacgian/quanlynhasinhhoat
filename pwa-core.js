...
document.addEventListener('DOMContentLoaded', function onDOMReady() {
  ...
  if (!isIOS && !isStandalone) {
    if (!hasInstalled()) {
      ...
      // Lắng nghe beforeinstallprompt (chỉ có trên Chrome/Edge/Samsung Internet)
      window.addEventListener('beforeinstallprompt', (e) => {
        e.preventDefault();
        deferredPrompt = e;
        
        // Hiện overlay sau OVERLAY_DELAY nếu chưa hiện
        if (!shown) {
      ...
      // Fallback: hiện overlay sau 3 giây cho tất cả các browser
      // (kể cả khi không có beforeinstallprompt - Firefox, Safari desktop, etc.)
      setTimeout(showAndroid, 3000);

      // Xử lý khi app được cài đặt thành công
      window.addEventListener('appinstalled', () => {
        hideOverlay();
        markAsInstalled();
      });

      // Xử lý nút cài đặt cho browser không hỗ trợ beforeinstallprompt
      document.getElementById('pwa-btn-install')?.addEventListener('click', () => {
        if (!deferredPrompt) {
          // Fallback cho browser không hỗ trợ: hiển thị hướng dẫn thủ công
          alert('Vui lòng mở menu trình duyệt (⋮ hoặc ☰) và chọn "Cài đặt ứng dụng" hoặc "Thêm vào Màn hình chính"');
          hideOverlay();
        }
      });
    }
  }
  // isStandalone === true → app đã cài → không hiện gì
