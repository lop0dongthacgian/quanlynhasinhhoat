...
document.addEventListener('DOMContentLoaded', function onDOMReady() {
  ...
  /* ── iOS: hiện overlay (tách riêng — iOS không có beforeinstallprompt) ── */
  ...
  /* ── Android / PC: beforeinstallprompt + fallback 3s ── */
  if (!isIOS && !isStandalone) {
    // Chỉ bỏ qua khi user đã bấm "Cài đặt" và xác nhận (outcome === 'accepted')
    // KHÔNG dùng hasInstalled() để chặn fallback — Chrome có thể không fire
    // beforeinstallprompt dù localStorage đã bị xóa, nên fallback phải luôn chạy
    if (!hasInstalled()) {
      ...
      // Fallback: Chrome đôi khi không fire beforeinstallprompt dù chưa cài
      // (ví dụ: từng từ chối, hoặc đã cài rồi xoá) → vẫn hiện overlay hướng dẫn thủ công
      // Fallback này chạy NGOÀI hasInstalled() để đảm bảo overlay luôn hiện
      // khi chưa ở standalone mode
      setTimeout(showAndroid, 3000);
      ...
      // hasInstalled() = true nhưng không ở standalone → có thể đã cài rồi xoá app
      // Xoá flag cũ để lần sau vào link sẽ hiện lại overlay
      try { localStorage.removeItem('pwa-installed'); } catch (_) {}
    }
  }
  // isStandalone === true → app đang chạy standalone (đã cài) → không hiện gì
