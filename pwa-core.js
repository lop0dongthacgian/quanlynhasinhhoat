...
document.addEventListener('DOMContentLoaded', function onDOMReady() {
  ...
  /* ─────────────────────────────────────────────────────────────
     LOGIC HIỂN THỊ OVERLAY (ĐÃ SỬA)
     - KHÔNG hiện nếu đang ở chế độ standalone (đã cài app)
     - CÓ hiện nếu chưa cài hoặc đã cài rồi xoá
  ───────────────────────────────────────────────────────────── */
  
  // QUAN TRỌNG: Nếu đang ở standalone mode (đã cài app) → KHÔNG hiện overlay
  if (isStandalone) {
    console.info('[PWA] App đang chạy ở chế độ standalone — không hiện overlay');
    return; // DỪNG hoàn toàn — không xử lý gì thêm
  }
  ...
  if (isIOS) {
    if (!hasInstalled()) {
      ...
      // Đã có flag cài đặt nhưng không ở standalone → đã xoá app
      // Xoá flag cũ và hiện lại overlay
  ...
  if (!isIOS) {
    ...
    // Nếu đã có flag cài đặt nhưng không ở standalone → đã xoá app
    // Xoá flag cũ để hiện lại overlay
    if (hasInstalled()) {
      try { localStorage.removeItem('pwa-installed'); } catch (_) {}
    }
    ...
    // Fallback: luôn hiện overlay sau 3 giây (cho dù beforeinstallprompt có fire hay không)
    setTimeout(showAndroid, 3000);

    // Khi app được cài đặt thành công
